...
/**
* @title Cosmic Universe NFT Staking v1.9.0
* @author @DirtyCajunRice
*/
contract ProfessionStakingHarmony is Initializable, PausableUpgradeable, AccessControlUpgradeable, IERC721ReceiverUpgradeable {
    ...
    modifier whenNotFinished() {
        require(LAST_REWARD_TIME > block.timestamp, "Character staking finished");
        _;
    }
    ...
    function enableStaking(address nftAddress, uint256 tokenId) public whenNotFinished onlyNotStaked(nftAddress, tokenId) {
    ...
    function batchEnableStaking(address[] memory nftAddresses, uint256[] memory tokenIds) public whenNotFinished {
    ...
    function stake(address nftAddress, uint256 tokenId) public whenNotFinished
    ...
    function batchStake(address[] memory nftAddresses, uint256[] memory tokenIds) public whenNotFinished {
        require(nftAddresses.length == tokenIds.length, "address count must match token count");
        for (uint256 i = 0; i < nftAddresses.length; i++) {
    ...
    function unstake(address nftAddress, uint256 tokenId) public {
        _unstake(_msgSender(), nftAddress, tokenId);
    }
    ...
    onlyStaked(nftAddress, tokenId) onlyUnlocked(nftAddress, tokenId) {
        require(_data[_address].nfts.length > 0, "No NFTs staked");
        NFT[] memory nfts = _data[_address].nfts;
        delete _data[_address].nfts;

        for (uint256 i = 0; i < nfts.length; i++) {
            if (nfts[i]._address == nftAddress && nfts[i].tokenId == tokenId) {
                _disburse_nft_reward(_address, nftAddress, tokenId, nfts[i].rewardFrom);
                delete _training_status[_address][nftAddress][tokenId];
            } else {
                _data[_address].nfts.push(NFT({
        ...
        _claim(_address);
        _clear_if_empty(_address);

        IERC721Upgradeable(nftAddress).transferFrom(address(this), _address, tokenId);

        emit Unstaked(_address, nftAddress, tokenId);
    }
    ...
    function _claim(address _address) internal {
        ...
        if (contractBalance >= amountToClaim) {
    ...
    function claimAndUnstakeForUser(address _address) public onlyRole(DEFAULT_ADMIN_ROLE) {
        TrainingStatus[] memory ts = getActiveTrainingOf(_address);
        require(ts.length == 0, "User has active training");
        NFT[] memory nfts = getStakedOf(_address);
        for (uint256 i = 0; i < nfts.length; i++) {
            _unstake(_address, nfts[i]._address, nfts[i].tokenId);
        }
    }

    function batchClaimAndUnstakeForUser(address[] memory addresses) public onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i = 0; i < addresses.length; i++) {
