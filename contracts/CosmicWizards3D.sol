...
contract CosmicWizard is ERC721Enumerable, Ownable {

    using SafeMath for uint256;

    // Token detail
    struct CosmicWizardDetail {
    ...
    // Token Detail
    mapping(uint256 => CosmicWizardDetail) private _cosmicWizard;
    ...
    function mintTokenId(uint256 tokenId) public onlyOwner {
        ...
        uint256 first_encounter = block.timestamp;
        _safeMint(msg.sender, tokenId);
        _cosmicWizard[tokenId] = CosmicWizardDetail(first_encounter);
        emit TokenMinted(tokenId, msg.sender, first_encounter);
    }
    ...
    /**
     * Mint CosmicWizard World
     */
    function mintCosmicWizard(uint256 numberOfTokens) public payable {    
     ...
     */
    function getCosmicWizardDetail(uint256 tokenId)
        public
        view
        returns (CosmicWizardDetail memory detail)
    {
        require(_exists(tokenId), "Token was not minted");

        return _cosmicWizard[tokenId];
    }
